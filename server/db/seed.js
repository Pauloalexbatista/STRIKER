...
export function seedData() {
  ...
  // 2. Seed Users with initial 0.00 balance
  const usersCount = db.prepare('SELECT COUNT(*) as count FROM users').get().count;
  if (usersCount === 0) {
    const users = [
      { id: 'u1', name: 'Paulo (Você)', favorite_club: 'SCP', balance: 0.00, avatar: '🦁' },
      { id: 'u2', name: 'Miguel Costa', favorite_club: 'SLB', balance: 0.00, avatar: '🦅' },
      { id: 'u3', name: 'Tiago Silva', favorite_club: 'FCP', balance: 0.00, avatar: '🐉' },
      { id: 'u4', name: 'André Matos', favorite_club: 'SCP', balance: 0.00, avatar: '🦁' },
      { id: 'u5', name: 'Rita Ferreira', favorite_club: 'SLB', balance: 0.00, avatar: '🦅' },
      { id: 'u6', name: 'João Pinho', favorite_club: 'FCP', balance: 0.00, avatar: '🐉' },
      { id: 'u7', name: 'Carlos Reis', favorite_club: 'SCB', balance: 0.00, avatar: '⚔️' },
      { id: 'u8', name: 'Nuno Santos', favorite_club: 'SCP', balance: 0.00, avatar: '🦁' },
      { id: 'u9', name: 'Inês Vieira', favorite_club: 'SLB', balance: 0.00, avatar: '🦅' },
      { id: 'u10', name: 'Rui Rocha', favorite_club: 'FCP', balance: 0.00, avatar: '🐉' }
    ];
  ...
  if (gamesCount === 0) {
    const now = Date.now();
    // Game 1: UPCOMING (Starts in 45 mins) - Sporting CP vs SC Braga
    const t1 = new Date(now + 45 * 60 * 1000).toISOString();
    // Game 2: LIVE (Started 30 mins ago) - SL Benfica vs FC Porto (O Clássico!)
    const t2 = new Date(now - 30 * 60 * 1000).toISOString();
    // Game 3: FINISHED (Played yesterday) - Vitória SC vs Famalicão
    const t3 = new Date(now - 24 * 60 * 60 * 1000).toISOString();
    // Game 4: UPCOMING (Tomorrow) - Gil Vicente vs Rio Ave
    ...
    insertGame.run('g1', 25, 'SCP', 'SCB', t1, 'UPCOMING', null, null, null, 0.00);
    ...
    // g2 bets (LIVE - started, names revealed, each deducted -1.00 pt)
    insertPred.run('p9', 'u1', 'g2', 'DRAW', nowIso, 1, 0.00, -1.00);
    insertPred.run('p10', 'u2', 'g2', 'HOME', nowIso, 1, 0.00, -1.00);
    insertPred.run('p11', 'u3', 'g2', 'AWAY', nowIso, 1, 0.00, -1.00);
    insertPred.run('p12', 'u4', 'g2', 'DRAW', nowIso, 1, 0.00, -1.00);
    insertPred.run('p13', 'u5', 'g2', 'HOME', nowIso, 1, 0.00, -1.00);
    insertPred.run('p14', 'u6', 'g2', 'AWAY', nowIso, 1, 0.00, -1.00);
    insertPred.run('p15', 'u7', 'g2', 'HOME', nowIso, 1, 0.00, -1.00);
    insertPred.run('p16', 'u8', 'g2', 'DRAW', nowIso, 1, 0.00, -1.00);
    insertPred.run('p17', 'u9', 'g2', 'HOME', nowIso, 1, 0.00, -1.00);
    insertPred.run('p18', 'u10', 'g2', 'AWAY', nowIso, 1, 0.00, -1.00);

    // g3 bets (FINISHED - Vitória SC won 2-1 HOME, 10 players, 2 winners: u1 and u4 got +5.00 brutos = +4.00 net!)
    insertPred.run('p19', 'u1', 'g3', 'HOME', nowIso, 1, 5.00, 4.00);
    insertPred.run('p20', 'u4', 'g3', 'HOME', nowIso, 1, 5.00, 4.00);
    ...
    // Update balances based on g2 and g3
    db.prepare('UPDATE users SET balance = 3.00 WHERE id = ?').run('u1');
    db.prepare('UPDATE users SET balance = 3.00 WHERE id = ?').run('u4');
    db.prepare('UPDATE users SET balance = -2.00 WHERE id NOT IN (?, ?)').run('u1', 'u4');
  }
}
